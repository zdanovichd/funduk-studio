...
const MenuItem = ({ item, level = 0, activeMenus, toggleMenu, isMobile }) => {
  ...
  const itemClass = level === 0
    ? `${styles.nav_menu__item} ${activeMenus[menuId] ? styles._active : ''} ${hasChildren ? styles.nav_menu__item_has_children : ''}`
    : level === 1
      ? `${styles.sub_menu__item} ${activeMenus[menuId] ? styles._active : ''} ${hasChildren ? styles.sub_menu__item_has_children : ''}`
    ...
    {(isLink || hasChildren) && (
      ...
      {hasChildren && (
        <ul
          ...
          style={isMobile ? { display: activeMenus[menuId] ? 'block' : 'none' } : undefined}
        >
          {data
...
export default function Header() {
  ...
  const repairServices = data.filter(item => item.parent === "remont-kvartir");
  ...
  return (
    <>
        ...
        <div className={styles.header__top}>
          <div className={styles.container}>
            <div className={styles.header__inner}>
              ...
              <div className={`${styles.header__info} ${styles.header__info_worktime}`}>
                <div className={styles.info_block}>
                  <p className={styles.info_block__title}>Режим работы</p>
                  <p className={styles.info_block__text}>Пн-Пт: c 10:00 до 18:00</p>
                </div>
              </div>

              <div className={`${styles.header__info} ${styles.header__info_question}`}>
                <div className={styles.info_block}>
                  <p className={styles.info_block__title}>Есть вопрос?</p>
                  <p className={styles.info_block__text}>Задайте его напрямую</p>
                </div>
              </div>
      ...
      {/* Основное меню */}
      <nav
        className={`${styles.header__nav} ${navHidden ? styles._hidden : ""}`}
        ref={navRef}
      >
        <div className={styles.container}>
          <ul className={styles.nav_menu} ref={menuRef}>
            <li
              ...
              onMouseEnter={() => !isMobile && toggleMenu('services', true)}
              onMouseLeave={() => !isMobile && toggleMenu('services', false)}
            >
              <Link
                href="/uslugi"
            ...
            <li className={styles.nav_menu__item}>
              <Link href="/about" className={styles.nav_menu__link}>
                О компании
              </Link>
            </li>
            <li className={styles.nav_menu__item}>
              <Link href="#" className={styles.nav_menu__link}>
                Наши работы
              </Link>
            </li>
            <li className={styles.nav_menu__item}>
            ...
            <li className={styles.nav_menu__item}>
              <Link href="/blog" className={styles.nav_menu__link}>
                Блог
              </Link>
            </li>
            <li className={styles.nav_menu__item}>
            ...
            <li className={styles.nav_menu__item}>
              <Link href="/contacts" className={styles.nav_menu__link}>
                Контакты
              </Link>
            </li>
          </ul>
          ...
          {/* Информационные блоки */}
          <div className={`${styles.nav__info} ${styles.nav__info_worktime}`}>
            <div className={styles.info_block}>
              <p className={styles.info_block__title}>Режим работы</p>
              <p className={styles.info_block__text}>Пн-Пт: c 10:00 до 18:00</p>
            </div>
          ...
          <div className={`${styles.nav__info} ${styles.nav__info_question}`}>
            <div className={styles.info_block}>
              <p className={styles.info_block__title}>Есть вопрос?</p>
              <p className={styles.info_block__text}>Задайте его напрямую</p>
            </div>
